...
const TYPE_LABELS = { site: 'Site touristique', hotel: 'Hébergement', nature: 'Nature & Réserve', culture: 'Culture & Arts' }
const TYPE_COLORS = { site: '#C1272D', hotel: '#006B3C', nature: '#D4A017', culture: '#7C3BBF' }
      ...
      <div className="min-h-screen pt-nav bg-blanc">
        <div className="max-w-4xl mx-auto px-4 py-12 animate-pulse space-y-6">
          ...
          <div className="space-y-2">
            {[...Array(4)].map((_, i) => <div key={i} className="h-4 bg-sable rounded" />)}
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen pt-nav bg-blanc flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/destinations" className="text-rouge hover:underline">← Retour aux destinations</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen pt-nav bg-blanc">
      ...
      <div className="relative h-80 md:h-96 overflow-hidden bg-nuit">
        {heroImage ? (
          <img src={heroImage} alt={place.name} className="w-full h-full object-cover opacity-70" />
        ) : (
          <div className="w-full h-full" style={{ background: `linear-gradient(135deg, ${color}33, #160A00)` }} />
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="font-serif text-3xl md:text-5xl text-blanc">{place.name}</h1>
            {place.region?.name && (
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex-1">
            <Link href="/destinations" className="inline-flex items-center gap-1 text-sm text-gris hover:text-rouge mb-6 transition-colors">
            ...
            {place.rating > 0 && (
              <div className="flex items-center gap-3 mb-6 p-4 bg-sable rounded-card">
                <div className="text-3xl font-serif text-or">{place.rating.toFixed(1)}</div>
                <div>
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <svg key={s} width="16" height="16" viewBox="0 0 24 24" fill={s <= Math.round(place.rating) ? '#D4A017' : '#EDD9A3'}>
                        <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                      </svg>
                    ))}
                  </div>
                  <p className="text-xs text-gris mt-0.5">{place.reviewCount} avis</p>
                </div>
              </div>
            )}
            ...
            {place.tags?.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-6">
                {place.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 bg-sable text-terre text-sm rounded-pill border border-sable-2">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <aside className="md:w-64 shrink-0">
            ...
            {place.lat && place.lng && (
              <div className="border border-sable-2 rounded-card overflow-hidden mb-4">
                ...
                <div className="bg-nuit/5 p-4 text-sm text-nuit space-y-1">
                  <p>Latitude : <span className="font-mono text-xs">{place.lat.toFixed(4)}</span></p>
                  <p>Longitude : <span className="font-mono text-xs">{place.lng.toFixed(4)}</span></p>
                </div>
                <Link
            ...
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${color}, #160A00)` }}
            >
              <p className="font-serif text-lg mb-2">Planifier votre visite</p>
              <p className="text-xs opacity-80 mb-4">Intégrez ce lieu dans votre itinéraire personnalisé.</p>
              <Link
