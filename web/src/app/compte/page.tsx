      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center gap-5 mb-10 p-6 bg-sable rounded-card border border-sable-2">
          ...
          <div className="flex-1">
            <h1 className="font-serif text-2xl text-nuit">{user?.firstName} {user?.lastName}</h1>
            ...
            <span className="inline-block mt-1 px-2.5 py-0.5 bg-blanc border border-sable-2 rounded-pill text-xs text-gris capitalize">
              {user?.role === 'owner' ? '🏨 Propriétaire' : '🧳 Voyageur'}
            </span>
          </div>
          <button
        ...
        {isLoading ? (
          <div className="space-y-4">
            {[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-sable rounded-card animate-pulse" />)}
          </div>
        ) : reservations.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-sable-2 rounded-card">
            <p className="text-3xl mb-3">🏨</p>
            <p className="text-gris">Vous n'avez aucune réservation pour le moment.</p>
            <a href="/reservation" className="inline-block mt-4 px-5 py-2.5 bg-rouge text-blanc rounded font-medium hover:bg-rouge/90 transition-colors text-sm">
          ...
          <div className="space-y-4">
            {reservations.map((r) => {
              ...
              return (
                <div key={r.id} className="p-5 border border-sable-2 rounded-card bg-blanc flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gris mt-0.5 capitalize">
                      {r.establishment?.type} · {r.establishment?.place?.region?.name}
                    </p>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className={`px-3 py-1 rounded-pill text-xs font-medium ${st.bg} ${st.text}`}>
                      {st.label}
                    </span>
                  </div>
                </div>
