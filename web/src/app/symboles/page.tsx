    ...
    <div className="min-h-screen bg-blanc pt-nav">
      ...
      <div className="bg-gradient-to-br from-nuit via-brun to-terre py-14">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <span className="text-or text-sm font-medium uppercase tracking-widest">Identité nationale</span>
          <h1 className="font-serif text-4xl md:text-5xl text-blanc mt-3">Symboles du Burkina</h1>
          <p className="text-sable-2 mt-4 max-w-xl mx-auto leading-relaxed">
            Les emblèmes, légendes et symboles culturels qui forgent l'identité du Pays des Hommes Intègres.
          </p>
          {/* Mini flag */}
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="rounded-card bg-sable animate-pulse h-52" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {symbols.map((symbol, i) => (
              ...
              >
                {/* Icon */}
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-rouge/10 to-or/10 border border-sable-2 flex items-center justify-center shrink-0 text-xl">
                  {['🐎', '🇧🇫', '🎵', '🎨', '🎭', '🌙'][i % 6]}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h3 className="font-serif text-xl text-nuit">{symbol.name}</h3>
                    {symbol.category && (
                  ...
                  <p className="text-sm text-gris leading-relaxed">{symbol.description}</p>
                </div>
              </div>
            ))}
          </div>
