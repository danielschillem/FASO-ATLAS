...
export default function InteractiveMap() {
  ...
  const { data: geojson, isLoading } = useQuery<GeoJSONCollection>({
    ...
    queryFn: async () => {
      const res = await mapApi.getPlaces({ type: activeFilter === 'all' ? undefined : activeFilter })
      return res.data
    },
  })
    ...
    <div className="flex h-[calc(100vh-72px)] mt-nav overflow-hidden">
      ...
      <aside className="w-72 bg-blanc border-r border-sable-2 flex flex-col overflow-y-auto shrink-0">
        <div className="p-4 border-b border-sable-2">
          <h2 className="font-serif text-xl text-nuit mb-1">Carte interactive</h2>
          <p className="text-xs text-gris">Burkina Faso · 13 régions</p>
        </div>

        {/* Filters */}
        <div className="p-4 space-y-2">
          <p className="text-xs font-medium text-gris uppercase tracking-wider mb-3">Filtrer par type</p>
          <button
            onClick={() => setActiveFilter('all')}
            className={clsx(
              'w-full text-left px-3 py-2 rounded text-sm transition-colors',
              activeFilter === 'all' ? 'bg-nuit text-blanc' : 'text-nuit hover:bg-sable'
            )}
          >
            Tous les lieux
          </button>
          {(Object.keys(PIN_COLORS) as PlaceType[]).map((type) => (
            <button
              ...
              className={clsx(
                ...
                activeFilter === type ? 'bg-nuit text-blanc' : 'text-nuit hover:bg-sable'
              )}
            >
              <span
        ...
        {selectedFeature && (
          <div className="mt-auto border-t border-sable-2 p-4">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${PIN_COLORS[selectedFeature.properties.type]}, #160A00)` }}
            >
              <p className="font-semibold">{selectedFeature.properties.name}</p>
              <p className="text-xs opacity-80">{selectedFeature.properties.region}</p>
            </div>
            <p className="text-xs text-gris line-clamp-3">{selectedFeature.properties.description}</p>
            <div className="flex items-center gap-2 mt-2">
              <span className="text-or text-sm">★ {selectedFeature.properties.rating.toFixed(1)}</span>
              <span className="text-xs text-gris">({selectedFeature.properties.reviewCount} avis)</span>
            </div>
          </div>
        )}
      </aside>
