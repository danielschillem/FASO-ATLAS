...
export function ReservationModal({ establishment: e, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        className="absolute inset-0 bg-nuit/60 backdrop-blur-sm"
        onClick={onClose}
      />
      <div className="relative bg-blanc rounded-card shadow-faso w-full max-w-md z-10">
        {success ? (
          <div className="p-8 text-center">
            <div className="w-14 h-14 bg-vert/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle className="w-8 h-8 text-vert" />
            </div>
            <h3 className="font-serif text-2xl text-nuit mb-2">
              Réservation envoyée !
            </h3>
            <p className="text-gris mb-6">
              Votre demande a été transmise. L'établissement vous contactera
              pour confirmer.
            </p>
            <button
              onClick={onClose}
              className="px-6 py-2.5 bg-vert text-blanc rounded font-medium hover:bg-vert/90 transition-colors"
            >
              Fermer
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex items-start justify-between p-5 border-b border-sable-2">
              ...
              <button
                onClick={onClose}
                className="text-gris hover:text-nuit transition-colors p-1"
              >
                <svg
                  width="18"
                  height="18"
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gris font-medium block mb-1">
                    Arrivée *
                  </label>
                  <input
                    type="date"
              ...
              {nights > 0 && e.priceMinFcfa > 0 && (
                <div className="bg-sable rounded p-3 text-sm space-y-1">
                  <div className="flex justify-between text-gris">
                    <span>
                      {e.priceMinFcfa.toLocaleString("fr-FR")} FCFA × {nights}{" "}
                      nuit{nights > 1 ? "s" : ""}
                    </span>
