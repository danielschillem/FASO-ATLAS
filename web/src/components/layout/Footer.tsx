...
export function Footer() {
  return (
    <footer className="bg-nuit text-sable-2 pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          ...
          <div className="col-span-1 md:col-span-1">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="font-serif text-lg text-blanc">Faso <span className="text-or">Atlas</span></span>
            </div>
            <p className="text-sm text-gris leading-relaxed">
              La plateforme de référence pour explorer le tourisme, le patrimoine et la culture du Burkina Faso.
            </p>
            {/* Mini flag */}
          ...
          {COLUMNS.map((col) => (
            <div key={col.title}>
              <h4 className="text-blanc font-medium text-sm mb-4 uppercase tracking-wider">{col.title}</h4>
              <ul className="space-y-2">
                {col.links.map(({ href, label }) => (
                  <li key={href}>
                    <Link href={href} className="text-sm text-gris hover:text-sable transition-colors">
                      {label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-gris">
          <span>© {new Date().getFullYear()} Faso Atlas. Tous droits réservés.</span>
