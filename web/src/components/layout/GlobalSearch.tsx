  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div className="hidden sm:flex items-center gap-1.5">
          {CATEGORY_FILTERS.map(({ value, label }) => (
            <button
              key={value}
              onClick={() => { setCategory(value); if (query.trim().length >= 2) search(query, value) }}
              className={clsx(
      ...
      {open && results && totalResults > 0 && (
        <div className="absolute left-0 right-0 bg-blanc border-b border-sable-2 shadow-faso max-h-80 overflow-y-auto">
          <div className="max-w-7xl mx-auto px-4 py-2 divide-y divide-sable-2">
            {/* Places */}
            {(results.places?.length ?? 0) > 0 && (
              <div className="py-2">
                <p className="text-xs text-gris font-medium uppercase tracking-wider mb-2">Lieux</p>
                {results.places!.slice(0, 4).map((p) => (
                  ...
                  >
                    <span
                      ...
                      style={{ backgroundColor: TYPE_COLORS[p.type] ?? '#8A7060' }}
                    />
            ...
            {(results.establishments?.length ?? 0) > 0 && (
              <div className="py-2">
                <p className="text-xs text-gris font-medium uppercase tracking-wider mb-2">Hébergements</p>
                {results.establishments!.slice(0, 3).map((e) => (
                  ...
                  >
                    ...
                    <span className="text-sm text-nuit flex-1">{e.place?.name}</span>
                    <span className="text-xs text-vert">{e.priceMinFcfa?.toLocaleString('fr-FR')} FCFA</span>
                  </Link>
                ))}
              </div>
            )}
            ...
            {(results.wiki?.length ?? 0) > 0 && (
              <div className="py-2">
                <p className="text-xs text-gris font-medium uppercase tracking-wider mb-2">Wiki Faso</p>
            ...
            {(results.itineraries?.length ?? 0) > 0 && (
              <div className="py-2">
                <p className="text-xs text-gris font-medium uppercase tracking-wider mb-2">Itinéraires</p>
                {results.itineraries!.slice(0, 3).map((it) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-gris">{it.durationDays}j</span>
                  </Link>
                ))}
              </div>
            )}
