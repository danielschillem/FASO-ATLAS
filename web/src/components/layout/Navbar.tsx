...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 h-nav bg-nuit/95 backdrop-blur-sm border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated() ? (
            <>
              <Link href="/compte" className="text-sm text-sable-2 hover:text-blanc transition-colors">
                {user?.firstName}
              </Link>
              <button
                onClick={logout}
            ...
            <>
              <Link href="/login" className="text-sm text-sable-2 hover:text-blanc transition-colors">
                Connexion
              </Link>
              <Link
                href="/register"
