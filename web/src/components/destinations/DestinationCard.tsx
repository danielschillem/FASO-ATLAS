  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-sable">
        ...
        {/* Rating */}
        {place.rating > 0 && (
          <span className="absolute bottom-3 right-3 px-2 py-1 bg-nuit/80 backdrop-blur-sm rounded text-or text-xs font-medium flex items-center gap-1">
        ...
        {place.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {place.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="px-2 py-0.5 bg-sable text-terre text-xs rounded-pill">
                {tag}
              </span>
            ))}
          </div>
